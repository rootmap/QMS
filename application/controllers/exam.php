        ...
        if ($this->input->post('exam_is_started')) {
            ...
            if ($exam_is_started != 1) {
                ...
                if ( ! $this->session->userdata('exam_is_started') ) {
                    ...
                    $getExamDetail=$this->Select_global_model->FlyQuery(array("SELECT  
                                                                                exm_exams.exam_random_qus
                                                                                FROM exm_user_exams 
                                                                                LEFT JOIN exm_exams ON exm_user_exams.exam_id=exm_exams.id
                                                                                WHERE exm_user_exams.id=".$user_exam_id));

                    //print_r_pre($getExamDetail);

                    $examQRT=0;
                    if($getExamDetail[0]['exam_random_qus']=="random")
                    {
                        $examQRT=1;
                    }
                    ...
                    $exam->exam_questions = $this->user_exam_question_model->get_user_exam_questions($user_exam_id,$examQRT);
                    ...
                    for ($i=0; $i<count($exam->exam_questions); $i++) {
                        ...
                        //print_r_pre($exam->exam_questions[$i]->qus_set);

                        $getQuesMarkInfo=$this->Select_global_model->FlyQuery(array("SELECT question_mark FROM exm_question_set_question_map WHERE question_set_id='".$exam->exam_questions[$i]->qus_set."' AND question_id='".$exam->exam_questions[$i]->question_id."'"));

                        $score_per_question = number_format($exam->exam_score / $exam->exam_total_questions, 2);
                        $exam->exam_questions[$i]->question->ques_score = $getQuesMarkInfo[0]['question_mark'];
                    ...
                    //print_r_pre($exam->current_question); die();
        ...
        if ($exam_is_mcq) {
           ...
           /// print_r_pre($exam->exam_questions)."<br>";
            for ($i=0; $i<count($exam->exam_questions); $i++) {
                ...
                //print_r_pre($exam->exam_questions[$i]->question);
                ...
                if ($ques_answer_type == 'dontknow') {
                    ...
                    $ques_user_score = $exam->exam_questions[$i]->question->ques_score;
                    $total_correct++;
