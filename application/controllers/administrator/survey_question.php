       ...
       /* $this->session->unset_userdata('filter_category');
        ...
        if ($records) {
            ...
            $tbl_heading = array(
                ...
                '11' => array('data'=> 'Action', 'class' => 'center', 'width' => '50')
            );
            ...
            for ($i = 0; $i<count($records); $i++) {
                ...
                $category_str = '';
                if ($records[$i]->category_id > 0) {
                    $category_str = $this->getCatNameByObject($records[$i]->category_id,"exm_survey_categories");
                }

                $subcategory_str = '';
                if ($records[$i]->sub_category_id > 0) {
                    $subcategory_str = $this->getCatNameByObject($records[$i]->sub_category_id,"exm_survey_sub_categories");
                }
                ...
                $tbl_row = array(
                    ...
                    '11' => array('data'=> $action_str, 'class' => 'center', 'width' => '50')
                );
    ...
    public function add_question()
    {
        ...
        if ($this->form_validation->run() == FALSE) {
            ...
            if ($res > 0) {
                ...
                redirect('administrator/survey_question/add');
            } else {
        ...
        if ($_FILES['question_file']['tmp_name'] != '' && $_FILES['question_file']['error'] == 0) {
            ...
            if ($file_error == '') {
                ...
                for ($i=$start; $i<=count($sheetData); $i++) {
                    ...
                    $survey_weight = $sheetData[$i]['L'];
                    $question_type = trim($sheetData[$i]['M']);
                    $question_choices = $sheetData[$i]['N'];
                   
                    $question_expiry_date = trim($sheetData[$i]['O']);
                            ...
                            if (count($question_choices_arr) < 2) {
                                ...
                                foreach ($question_choices_arr as $key => $value) {
                                    ...
                                    if(count($kval)>1)
                                    {
                                        ...
                                        $question_choices[$i]['text'] = trim($kval[0]);
                                        $question_choices[$i]['marks'] = trim($kval[1]);
                                    }
                                    else
                                    {
                                        $question_choices[$i] =  array();
                                        $question_choices[$i]['text'] = $value;
                                        $question_choices[$i]['marks'] = $value;
                                    }
                                    //$question_choices[$i]['marks'] = $value;
                                    $i++;
                                }
