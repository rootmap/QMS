...
class Exam extends MY_Controller
{
  ...
  WHERE qsm.question_set_id='.$set_id)
        ...
        $getExamCenterInfo=$this->select_global_model->FlyQuery(array("SELECT 
                                                                        exm_exam_venue_map.venue_id, 
                                                                        exm_venue.name,
                                                                        exm_venue.address
                                                                        FROM exm_exam_venue_map 
                                                                        LEFT JOIN exm_venue ON exm_exam_venue_map.venue_id=exm_venue.id
                                                                        WHERE exam_id=".$exam_id));

        $page_info['ExamCenterInfo'] = $getExamCenterInfo;
        ...
        $setInfo=$this->select_global_model->FlyQuery(array("SELECT * FROM exm_question_set WHERE id='$set_id'"));
        $page_info['setInfo'] = $setInfo;
...
/** new line here start 23-12-2017**/
     ...
     public function create_question_pool()
    {
        ...
        //$page_info['questionList'] = $this->select_global_model->select_array('question_set',array('id'=>$getData));
    ...
    public function loadQuestionByCat()
    {
        $ranstr=" ORDER BY ID ASC";
        
        if(!empty($this->input->post('random')))
        {
            $random=$this->input->post('random');
            if($random>0)
            {
                $ranstr=" ORDER BY NEWID()";
            }
        }
        ...
        $sub_four_cat_parent=$this->input->post('sub_four_cat_parent');
        ...
        if(!empty($this->input->post('category_id')))
        {
            ...
            {
                $whereString .="WHERE  category_id='$category_id'";
            }
        }

        if(!empty($this->input->post('sub_cat_parent')))
        {
            if(!empty($whereString))
            {
                $whereString .=" AND sub_category_id='$sub_cat_parent' ";
            }
            else
            {
                $whereString .="WHERE  sub_category_id='$sub_cat_parent' ";
            }
        }

        if(!empty($this->input->post('sub_two_cat_parent')))
        {
            if(!empty($whereString))
            {
                $whereString .=" AND sub_two_category_id='$sub_two_cat_parent'  ";
            }
            else
            {
                $whereString .="WHERE  sub_two_category_id='$sub_two_cat_parent'  ";
            }
        }

        if(!empty($this->input->post('sub_three_cat_parent')))
        {
            if(!empty($whereString))
            {
                $whereString .=" AND sub_three_category_id='$sub_three_cat_parent'   ";
            }
            else
            {
                $whereString .="WHERE  sub_three_category_id='$sub_three_cat_parent'   ";
            }
        }

        if(!empty($this->input->post('sub_four_cat_parent')))
        {
            if(!empty($whereString))
            {
                $whereString .=" AND sub_four_category_id='$sub_four_cat_parent'  ";
            }
            else
            {
                $whereString .="WHERE sub_four_category_id='$sub_four_cat_parent'  ";
            }
        }
        ...
        if(!empty($ques_type))
        {
            if(!empty($whereString))
            {
                $whereString .=" AND ques_type='$ques_type'  ";
            }
            else
            {
                $whereString .="WHERE ques_type='$ques_type'  ";
            }
        }


       // echo "SELECT exm_questions.*,(SELECT exm_categories.cat_name FROM exm_categories WHERE exm_categories.id=exm_questions.category_id) as cat FROM exm_questions ".$whereString." ".$ranstr." OFFSET 0 ROWS FETCH NEXT 10 ROWS ONLY";

        //$loadType,$category_id,$sub_cat_parent,$sub_two_cat_parent,$sub_three_cat_parent,$sub_four_cat_parent
        $json=$this->select_global_model->FlyQuery(array("SELECT exm_questions.*,(SELECT exm_categories.cat_name FROM exm_categories WHERE exm_categories.id=exm_questions.category_id) as cat FROM exm_questions ".$whereString." ".$ranstr." OFFSET 0 ROWS FETCH NEXT 10 ROWS ONLY"));
        echo json_encode($json); die();
    }

    public function surveyloadQuestionByCat()
    {
        $ranstr=" ORDER BY ID ASC";
        ...
        //$loadType,$category_id,$sub_cat_parent,$sub_two_cat_parent,$sub_three_cat_parent,$sub_four_cat_parent
        $json=$this->select_global_model->FlyQuery(array("SELECT exm_survey_questions.*,(SELECT exm_survey_categories.cat_name FROM exm_survey_categories WHERE exm_survey_categories.id=exm_survey_questions.category_id) as cat FROM exm_survey_questions ".$whereString." ".$ranstr." OFFSET 0 ROWS FETCH NEXT 10 ROWS ONLY"));
        echo json_encode($json); die();
    }
