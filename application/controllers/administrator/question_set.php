...
//print_r_pre($page_info['questionSet'][0]['random_qus']);die;
    ...
    public function export_answer(){
      ...
      $update_data = $this->insert_global_model->globalinsert($this->tbl_exam_users_activity,array('user_id'=>$this->logged_in_user->id,
        ...
        for($i=0; $i<count($questions); $i++){
          ...
          //$getExamCenterInfo=$this->select_global_\model->FlyQuery(array("SELECT 
            ...
            if($questions[$i]->ques_type=='mcq') {
                ...
                $ques_info[$index]['sl'] =$sl;
                $ques_info[$index]['category'] =$questions[$i]->cat_name;
                $ques_info[$index]['question']=htmlspecialchars_decode(str_replace("&nbsp;","",strip_tags(html_entity_decode($questions[$i]->ques_text))));
                

                $ques_info[$index]['correct_choice']='';
                ...
                foreach ($questions[$i]->ques_choices as $keyy=>$vaue) {
                    ...
                    if($vaue['is_answer']==1)
                    {
                        if(!empty($ques_info[$index]['correct_choice']))
                        {
                            $ques_info[$index]['correct_choice'] .=", ";
                            $ques_info[$index]['correct_choice'] .=chr($chr).")";
                        }
                        else
                        {
                            $ques_info[$index]['correct_choice'] .=chr($chr).")";
                        ...
                        if(!empty($ques_info[$index]['answer']))
                        {
                            $ques_info[$index]['answer'] .=", ";
                            $ques_info[$index]['answer'] .=chr($chr).") ".htmlspecialchars_decode($vaue['text']);
                        }
                        else
                        {
                            $ques_info[$index]['answer'] .=chr($chr).") ".htmlspecialchars_decode($vaue['text']);
  ...
  WHERE qsm.question_set_id='.$question_set_id)
        ...
        $setInfo=$this->select_global_model->FlyQuery(array("SELECT * FROM exm_question_set WHERE id='$question_set_id'"));
                ...
                <br><br>
                    <table  style="border: 1px #ccc solid;" cellpadding="5" cellspacing="0" width="100%" align="left">
                        ...
                        <tr>
                            ...
                            <td style="border: 1px #ccc solid;"><strong>'.$exam->exam_title.'</strong></td>
                        </tr>
                        <tr>
                            <td style="border: 1px #ccc solid;">
                              <strong>Exam ID</strong>
                            </td>
                            <td style="border: 1px #ccc solid;"><strong>'.$exam->id.'</strong></td>
                        </tr>
                        <tr>
                            <td style="border: 1px #ccc solid;">
                                <strong>Exam Centre</strong>
                            </td>
                            <td style="border: 1px #ccc solid;"><strong>'.$venues.'</strong></td>
                        </tr>
                        <tr>
                            <td style="border: 1px #ccc solid;">
                                <strong>Exam Location</strong>
                            </td>
                            <td style="border: 1px #ccc solid;"><strong>'.$venuesLocation.'</strong></td>
                        </tr>
                        <tr>
                            <td style="border: 1px #ccc solid;"> 
                        ...
                        <tr>
                            ...
                            <td style="border: 1px #ccc solid;">'.$user->nid_passport_no.'</td>
                        </tr>
                        <tr>
                            <td style="border: 1px #ccc solid;"> <strong> '.html_entity_decode('Candidates Signature').'</strong></td>
                            <td style="border: 1px #ccc solid;">';
                    ...
                    <table cellpadding="5" cellspacing="1" width="100%" align="center">
                        ...
                        <tr>
                            ...
                            <td width="35%" valign="middle" align="center">';
                                  ...
                                  $dataQrcCodeString="";
                                  $dataQrcCodeString .="Exam ID: ".$exam->id;
                                  ...
                                  $dataQrcCodeString .=", Questionnaire Set No.: ".$setInfo[0]['name'];
...
';
      ...
      $slqsPart=1;
      foreach($questionsCateWithMark as $index=>$catMark):
      ...
      $strHtml .='<table border="0" width="100%">
        ...
        $questions= $this->question_set_model->get_question_by_ques_cat_set_id($question_set_id,$catMark->category_id,$randomStatus);
        ...
        for($i=0; $i<count($questions); $i++){
            ...
            $quesNo=$i+1;
            $strHtml .=  '<tr><td valign="middle" width="3%">'.$quesNo. ') </td><td valign="middle" width="80%" align="left">' .$carTextStr . '</td><td valign="middle">('.$questions[$i]->mark.')</td></tr>';
            //$strHtml .='<br>';
    ...
    public function questionSetPreview()
    {
      ...
      $strHtml .='<table border="0" width="100%">
        ...
        //print_r_pre($questions);
        $strHtml .='<table border="0" width="100%"><tbody>';
        for($i=0; $i<count($questions); $i++){
            $quesNo=$i+1;
            $strHtml .=  '<tr><td width="3%">'.$quesNo. ')</td> <td width="90%">' .trim(html_entity_decode(trim($questions[$i]->ques_text))) . '</td><td>('.$questions[$i]->mark.')</td></tr>';
            ...
            {
              $strHtml .='<tr><td colspan="3"><div style="page-break-before:always"></div></td></tr>';
            }
