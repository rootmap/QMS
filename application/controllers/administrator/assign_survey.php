...
class Assign_survey extends MY_Controller
{
    ...
    public function bulkdownloadformat()
    {
         ...
         /*$survey[$i]['user_id'] = $login;
                ...
                $this->excel->getActiveSheet()->setCellValue('C1', 'Survey Type');
                $this->excel->getActiveSheet()->setCellValue('D1', 'Start Date');
                ...
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(0, $row,'User ID');
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(1, $row,'Survey ID');
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(2, $row, "Survey Type");
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(3, $row, 'Start Date');
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(4, $row, 'End Date');
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(5, $row, 'Status');

                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(0, 2,'amit_cand5');
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(1, 2,'25');
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(2, 2, "Normal");
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(3, 2, '01/04/2019');
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(4, 2, '01/04/2019');
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(5, 2, 'Open');

                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(0, 3,'amit_cand7');
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(1, 3,'25');
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(2, 3, "Anonymous");
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(3, 3, '01/04/2019');
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(4, 3, '01/04/2019');
                $this->excel->getActiveSheet()->setCellValueByColumnAndRow(5, 3, 'Open');
       
                    
                
                $filename = 'Assign Bulk Survey User Data Format '. date('Y-m-d') .'.xlsx';
        ...
        if ($_FILES['user_file']['tmp_name'] != '' && $_FILES['user_file']['error'] == 0) {
            ...
            if ($file_error == '') {
                ...
                for ($i=$start; $i<=count($sheetData); $i++) {
                    ...
                    $survey_user_id = trim($sheetData[$i]['A']);
                    $survey_id = trim($sheetData[$i]['B']);
                    $survey_type = trim($sheetData[$i]['C']);
                    $start_date = trim($sheetData[$i]['D']);
                    $end_date = trim($sheetData[$i]['E']);
                    $survey_status = trim($sheetData[$i]['F']);

                    if ($survey_user_id == '' && $survey_id == '' && $survey_type == '' && $start_date == '' && $end_date == '' && $survey_status == '') {
                        continue;
                    } else {
                        $survey[$i]['user_id'] = $survey_user_id;
                        $survey[$i]['survey_id'] = $survey_id;
                        $survey[$i]['survey_type'] = $survey_type;
                        $survey[$i]['start_date'] = $start_date;
                        $survey[$i]['end_date'] = $end_date;
                        $survey[$i]['status'] = strtolower($survey_status)?strtolower($survey_status):'open';
                    }
                }
                ...
                if (count($survey) > 0) {
                    foreach($survey as $row => $val) {
                        ...
                        if ($survey_id == '') {
                            $error_messages[$row][] = 'Survey id can not be empty';
                            $row_has_error = true;
                        } 
                        ...
                        if ($row_has_error) {
                            ...
                            if(strtolower(trim($survey_type))=="anonymous")
                            {
                                $survey[$row]['survey_anms'] = 'yes';
                            }
                            else
                            {
                                $survey[$row]['survey_anms'] = 'no';
                            }
                            ...
                            //$survey[$row]['is_approved'] = '1';
                            $survey[$row]['added'] = date("Y-m-d H:i:s");

                            unset($survey[$row]['survey_type']);
                        }
                    }
                }
