            ...
            foreach($filter as $key => $value) {
                if ($key == 'filter_expired') {
                    if ($value['value'] == 'expired') {
                        $this->db->where(' ques_expiry_date <= cast(getdate() as DATETIME2)  AND ques_expiry_date IS NOT NULL ', '', false);
                    } elseif ($value['value'] == 'available') {
                        $this->db->where(' ( ques_expiry_date > cast(getdate() as DATETIME2) OR ques_expiry_date IS NULL ) ', '', false);
                    }
        ...
        if ($query->num_rows() > 0) {
            ...
            if (is_array($filter) && count($filter) > 0) {
                foreach($filter as $key => $value) {
                    if ($key == 'filter_expired') {
                        if ($value['value'] == 'expired') {
                            $this->db->where(' ques_expiry_date <= cast(getdate() as DATETIME2) AND ques_expiry_date != \'0000-00-00 00:00:00\' AND ques_expiry_date IS NOT NULL ', '', false);
                        } elseif ($value['value'] == 'available') {
                            $this->db->where(' ( ques_expiry_date > cast(getdate() as DATETIME2) OR ques_expiry_date IS NULL ) ', '', false);
                        }
                    } elseif ($key == 'filter_question') {
