...
class Question_set_model extends CI_Model
{
    ...
    public function get_question_by_ques_set_id($ques_set_id,$rand=0)
    {
        ...
        if ($ques_set_id > 0) {
            $sql = 'select (SELECT cat_name FROM exm_categories WHERE id=category_id) as cat_name,ques_text,ques_choices,ques_type,mark from exm_questions where id in (select question_id from exm_question_set_question_map where question_set_id='.$ques_set_id.')';
            $res = $this->db->query($sql);
    ...
    public function get_question_by_ques_cat_set_id($ques_set_id,$cat_id=0,$randID=0)
    {
        ...
        if ($ques_set_id > 0) {
            ...
            $sql = "SELECT 
                    exm_questions.id,
                    exm_questions.ques_text,
                    exm_questions.ques_choices,
                    exm_questions.ques_type,
                    exm_question_set_question_map.question_mark as mark,
                    exm_question_set_question_map.is_mandatory
                    FROM exm_question_set_question_map 
                    INNER JOIN exm_questions ON exm_question_set_question_map.question_id=exm_questions.id 
                    WHERE exm_questions.category_id='".$cat_id."' AND exm_question_set_question_map.question_set_id='".$ques_set_id."'".$sqlStrOrder;
           // echo $sql; die();
            $res = $this->db->query($sql);
    ...
    public function get_question_category_by_ques_set_id($ques_set_id)
    {
        ...
        $ques_set_id = (int)$ques_set_id;
        if ($ques_set_id > 0) {
            $sql = 'select category_id,dbo.get_categoryName_by_id(category_id) as cat_name,dbo.get_categoryMark_by_id(category_id,'.$ques_set_id.') as total from exm_questions where id in (select question_id from exm_question_set_question_map where question_set_id='.$ques_set_id.') GROUP BY category_id  ORDER BY category_id';
            $res = $this->db->query($sql);
            //echo $this->db->last_query(); die();
            $result = $res->result();
            //var_dump($result);die;
    ...
    {
        $this->db->select("TA.*,TB.is_mandatory as mandatory_check,TB.question_mark as current_mark, (SELECT cat_name FROM exm_categories WHERE id=TA.category_id) as cat");
        $this->db->from('exm_questions TA');
        $this->db->order_by('TB.id','ASC');
