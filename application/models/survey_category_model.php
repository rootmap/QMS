...
class Survey_category_model extends CI_Model
{
    ...
    private $table_sub_name = 'survey_sub_categories';
    private $table_sub_two_name = 'survey_sub_two_categories';
    private $table_sub_three_name = 'survey_sub_three_categories';
    private $table_sub_four_name = 'survey_sub_four_categories';
    ...
    public function get_paged_subcategories($limit, $offset = 0, $filter = array())
    {
        ...
        $this->db->select('id,
                         cat_name,
                         (SELECT cat_name FROM exm_survey_categories WHERE exm_survey_categories.id=exm_survey_sub_categories.cat_parent) as cat_parent_name,
                         created_at,
                         dbo.get_user_login_name_by_id(created_by) as created_by
                        ');
        $query = $this->db->get('exm_survey_sub_categories', $limit, $offset);
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->db->order_by('id','DESC');
            $this->db->from('exm_survey_sub_categories');
    ...
    public function get_paged_subTwocategories($limit, $offset = 0, $filter = array())
    {
        ...
        if (is_array($filter) && count($filter) > 0) {
            foreach($filter as $key => $value) {
                if ($key == 'category') {
                    $this->db->or_where("((SELECT cat_name FROM exm_survey_categories WHERE exm_survey_categories.id=exm_survey_sub_categories.cat_parent) LIKE '%". $value['value'] ."%')", '', false);
                    $this->db->or_where("(cat_name LIKE '%". $value['value'] ."%')", '', false);
                    $this->db->or_where("(dbo.get_user_login_name_by_id(created_by) LIKE '%". $value['value'] ."%')", '', false);
                    $this->db->or_where("(created_at LIKE '%". $value['value'] ."%')", '', false);
                } else {
        ...
        $this->db->select('id,
                         cat_name,
                         (SELECT cat_name FROM exm_survey_categories WHERE exm_survey_categories.id=exm_survey_sub_two_categories.cat_parent) as cat_parent_name,
                         (SELECT cat_name FROM exm_survey_sub_categories WHERE exm_survey_sub_categories.id=exm_survey_sub_two_categories.sub_cat_parent) as sub_cat_parent_name,
                         created_at,
                         dbo.get_user_login_name_by_id(created_by) as created_by
                        ');
        $query = $this->db->get('exm_survey_sub_two_categories', $limit, $offset);
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->db->select('id,dbo.get_user_login_name_by_id(created_by) as created_by');
            $this->db->order_by('id','DESC');
            $this->db->from('exm_survey_sub_two_categories');
    ...
    public function get_pagedThreecategories($limit, $offset = 0, $filter = array())
    {
        ...
        $this->db->select('id,
                         cat_name,
                         (SELECT cat_name FROM exm_survey_categories WHERE exm_survey_categories.id=exm_survey_sub_three_categories.cat_parent) as cat_parent_name,
                         (SELECT cat_name FROM exm_survey_sub_categories WHERE exm_survey_sub_categories.id=exm_survey_sub_three_categories.sub_cat_parent) as sub_cat_parent_name,
                         (SELECT cat_name FROM exm_survey_sub_two_categories WHERE exm_survey_sub_two_categories.id=exm_survey_sub_three_categories.sub_two_cat_parent) as sub_two_cat_parent_name,
                         created_at,
                         dbo.get_user_login_name_by_id(created_by) as created_by
                        ');
        $query = $this->db->get('exm_survey_sub_three_categories', $limit, $offset);
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->db->select('id,dbo.get_user_login_name_by_id(created_by) as created_by');
            $this->db->order_by('id','DESC');
            $this->db->from('exm_survey_sub_three_categories');
    ...
    public function get_pagedFourcategories($limit, $offset = 0, $filter = array())
    {
        ...
        $this->db->select('id,
                         cat_name,
                         (SELECT cat_name FROM exm_survey_categories WHERE exm_survey_categories.id=exm_survey_sub_four_categories.cat_parent) as cat_parent_name,
                         (SELECT cat_name FROM exm_survey_sub_categories WHERE exm_survey_sub_categories.id=exm_survey_sub_four_categories.sub_cat_parent) as sub_cat_parent_name,
                         (SELECT cat_name FROM exm_survey_sub_two_categories WHERE exm_survey_sub_two_categories.id=exm_survey_sub_four_categories.sub_two_cat_parent) as sub_two_cat_parent_name,
                         (SELECT cat_name FROM exm_survey_sub_three_categories WHERE exm_survey_sub_three_categories.id=exm_survey_sub_four_categories.sub_three_cat_parent) as sub_three_cat_parent_name,
                         created_at,
                         dbo.get_user_login_name_by_id(created_by) as created_by
                        ');
        $query = $this->db->get('exm_survey_sub_four_categories', $limit, $offset);
        ...
        if ($query->num_rows() > 0) {
            ...
            $this->db->select('id,dbo.get_user_login_name_by_id(created_by) as created_by');
            $this->db->order_by('id','DESC');
            $this->db->from('exm_survey_sub_four_categories');
    ...
    public function get_subcategory($category_id)
    {
        ...
        if ($category_id > 0) {
            $this->db->where('id', $category_id);
            $query = $this->db->get($this->table_sub_name);
    ...
    public function get_subtwocategory($category_id)
    {
        $category_id = (int)$category_id;

        if ($category_id > 0) {
            $this->db->where('id', $category_id);
            $query = $this->db->get($this->table_sub_two_name);
    ...
    public function get_subthreecategory($category_id)
    {
        $category_id = (int)$category_id;

        if ($category_id > 0) {
            $this->db->where('id', $category_id);
            $query = $this->db->get($this->table_sub_three_name);
    ...
    public function get_subfourcategory($category_id)
    {
        $category_id = (int)$category_id;

        if ($category_id > 0) {
            $this->db->where('id', $category_id);
            $query = $this->db->get($this->table_sub_four_name);
    ...
    public function update_subCategory($category_id, $category)
    {
        $category_id = (int)$category_id;
        ...
        if ($category_id > 0) {
            ...
            $this->db->where('id', $category_id);
            $this->db->update($this->table_sub_name, $category);
    ...
    public function update_subTwoCategory($category_id, $category)
    {
        $category_id = (int)$category_id;
        ...
        if ($category_id > 0) {
            ...
            $this->db->where('id', $category_id);
            $this->db->update($this->table_sub_two_name, $category);
    ...
    public function update_subThreeCategory($category_id, $category)
    {
        $category_id = (int)$category_id;
        ...
        if ($category_id > 0) {
            ...
            $this->db->where('id', $category_id);
            $this->db->update($this->table_sub_three_name, $category);
    ...
    public function update_subFourCategory($category_id, $category)
    {
        $category_id = (int)$category_id;
        ...
        if ($category_id > 0) {
            ...
            $this->db->where('id', $category_id);
            $this->db->update($this->table_sub_four_name, $category);
