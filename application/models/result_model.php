...
//DATEADD(month,'.$duration .',GETDATE())
    ...
    public function get_attendee_list($exam_id = 0, $group_id = 0, $team_id = 0, $start_date = '', $end_date = '')
    {
        ...
        $this->db->select("TC.id as ID, 
                            CONCAT(TC.user_first_name, ' ', TC.user_last_name) as user_name,
                            TC.phone as phone, 
                            TC.user_login AS user_login,
                            TD.team_name AS team_name,
                            TA.ue_start_date AS ue_start_date,
                            TA.ue_start_time AS ue_start_time,
                            TA.ue_end_date AS ue_end_date,
                            TA.ue_end_time AS ue_end_time,
                            TA.ue_status AS ue_status");
        $this->db->from('exm_user_exams TA');
        $this->db->join('exm_users TC','TC.id = TA.user_id', 'inner');
        $this->db->join('exm_user_teams TD','TD.id = TC.user_team_id', 'left');
        ...
        if ($group_id > 0){
            ...
            if ($start_date != '' && $end_date != '') {
                $date_where = "(TA.ue_start_date BETWEEN '". $start_date ."' AND '".$end_date."')";
                $this->db->where($date_where);
            }
            $query = $this->db->get();
    ...
    public function get_attendee_list_count($exam_id = 0, $group_id = 0, $team_id = 0, $start_date = '', $end_date = '')
    {
        ...
        $this->db->from('exm_user_exams TA');
        $this->db->join('exm_users TC','TC.id = TA.user_id', 'left');
        $this->db->join('exm_user_teams TD','TD.id = TC.user_team_id', 'left');
        ...
        if ($group_id > 0){
            ...
            $this->db->where('TA.ue_status','complete');
            $query = $this->db->get();
    ...
    public function update_result_exam_state($result_id, $exam_state,$totalUserScore=0)
    {
        $result_id = (int)$result_id;
        ...
        if($totalUserScore>0)
        {
            $data = array(
                ...
                'result_user_score' => $totalUserScore
            );
        }
        else
        {
