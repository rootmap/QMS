...
class Exam_model extends CI_Model
{
    ...
    public function get_Set_Info($set_id)
    {
        ...
        if ($set_id > 0) {
            ...
            $this->db->order_by('TB.category_id');
            $this->db->from('exm_question_set_question_map as q');
            $this->db->select("TB.category_id,dbo.get_categoryName_by_id(TB.category_id) as cat_name,CONCAT(count(q.id),' X ',q.question_mark,' = ',(count(q.id)*q.question_mark)) as summary_row,
