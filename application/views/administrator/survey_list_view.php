    ...
    <div class="span12">
        ...
        <div class="row control-row control-row-top">
            ...
            <?php echo form_open('administrator/survey/filter', array('class' => 'form-inline', 'id' => 'filter-form')); ?>
                ...
                <select name="filter_approval_status" id="filter_approval_status" class="chosen-select">
                    <option value="">Approval Status</option>
                    <option 
                    <?php 
                    if(isset($this->form_data->filter_approval_status))
                    {
                        if($this->form_data->filter_approval_status==1)
                        {
                    ...
                    if(isset($this->form_data->filter_approval_status))
                    {
                        if($this->form_data->filter_approval_status==2)
                        {
                            ?> selected="selected" <?php
                        }
                    }
                    ...
                    if(isset($this->form_data->filter_approval_status))
                    {
                        if($this->form_data->filter_approval_status==3)
                        {
                            ?> selected="selected" <?php
                        }
                    }
